nb_wifi_bt <- read.csv("../csv/FILTERED_NB_WIFI_BT.csv",header=T,sep=",")
nb_wifi_bt <- t(nb_wifi_bt)

knn_wifi_bt <- read.csv("../csv/FILTERED_KNN_WIFI_BT.csv",header=T,sep=",")
knn_wifi_bt <- t(knn_wifi_bt)

rf_wifi_bt <- read.csv("../csv/FILTERED_RF_WIFI_BT.csv",header=T,sep=",")
rf_wifi_bt <- t(rf_wifi_bt)

nb_wifi <- read.csv("../csv/FILTERED_NB_WIFI.csv",header=T,sep=",")
nb_wifi <- t(nb_wifi)
...
png("../img/FILTERED_TOP_5.png",500,300)
